...
import com.alibaba.datax.plugin.writer.mongodb34writer.util.MongoUtil;
...
public class MongoDB34Writer extends Writer {
	...
	public static class Task extends Writer.Task {
		...
		public void prepare() {
			...
			if (Strings.isNullOrEmpty(database) || Strings.isNullOrEmpty(collection) || mongoClient == null
		...
		private void doBatchInsert(MongoCollection<BasicDBObject> collection, List<Record> writerBuffer,
				...
				for (int i = 0; i < record.getColumnNumber(); i++) {
					...
					if (Strings.isNullOrEmpty(columnAsString)) {
						if (KeyConstant.isArrayType(type.toLowerCase())) {
							data.put(columnName, new Object[0]);
						} else {
					...
					if (Column.Type.INT.name().equalsIgnoreCase(type)) {
						...
						try {
							if (KeyConstant.isObjectIdType(type.toLowerCase())) {
								data.put(columnName, new ObjectId(columnAsString));
			 ...
			 * 如果存在重复的值覆盖
			 */
			if (this.writeMode != null && this.writeMode.getString(KeyConstant.IS_REPLACE) != null
				...
				if (!Strings.isNullOrEmpty(uniqueKey)) {
					...
					for (BasicDBObject data : dataList) {
						...
						ReplaceOneModel<BasicDBObject> replaceOneModel = new ReplaceOneModel<BasicDBObject>(query,
